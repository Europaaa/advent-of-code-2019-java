...
public class Day18Part2 {
    ...
    public static Response evaluate(char[] expression, int start, long operand, char operator) {
        ...
        switch (operator) {
            case '+':
                response = getOperand(expression, start);
                value = response.result + operand;
                break;
            case '*':
                response = evaluate(expression, start, 0, '+');
                value = response.result * operand;
                break;
        }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        long sum = expressions.stream().mapToLong(Day18Part2::evaluate).sum();
