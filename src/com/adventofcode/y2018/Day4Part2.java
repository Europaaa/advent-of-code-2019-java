...
public class Day4Part2 {
    ...
    public static long calculate(List<String> logs) {
        ...
        Map<String, Long> counters = history.entrySet().stream()
                .collect(Collectors.toMap(
                        Map.Entry::getKey,
                        entry -> Arrays.stream(count(entry.getValue())).max().getAsLong()));

        String guard = counters.keySet().stream()
                .max(Comparator.comparing(counters::get))
                .get();

        // What minute does that guard spend asleep the most?
        long[] counter = count(history.get(guard));
        long max = counters.get(guard);
        int minute = IntStream.range(0, 60)
                .filter(i -> counter[i] == max)
                .findFirst()
                .getAsInt();

        return Long.parseLong(guard) * minute;
    }
    ...
    private static long[] count(List<TimeRange> timeRanges) {
